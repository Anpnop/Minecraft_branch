You'll need at least Java 17 (recommended: Java 21) to play 1.20.1 Modpacks. Recommended versions can be found on either:

Easier to install:
- https://adoptium.net/temurin/releases/     (Make sure to get the 64bit .msi file, if using Windows)

Better performance, for advanced users:
- https://www.graalvm.org/downloads/      (GraalVM has been shown to give slightly better Minecraft server performance, you can read details in the github link in the Q&A section)

If your server still doesn't launch with Java 21, the simplest fix is usually to uninstall other Java versions! These can be found by going to Programs and Features, then typing the letter "j" to search for versions of Java.
Note that some mods are currently INCOMPATIBLE with Java 22. 
...
A: Right click the Start.ps1 file, then click Run with Powershell. 
	Q: How do I join my server?
A: 1) Install the modpack using a modded launcher (Curseforge, ATLauncher, Prism, etc)	2) go to the Multiplayer tab in the main menu	3) click Direct Connect and type localhost
...
A: There are a few things you can do to for server lag:
 1) Follow this guide for optimal Java args. Please note that these can cause instability or worse performance in rare cases, so remove them if they don't work: https://github.com/Mukul1127/Minecraft-Performance-Flags-Benchmarks/tree/main
